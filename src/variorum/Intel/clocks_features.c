    ...
    if (!init)
    {
        ...
        if (control_domains == SOCKET)
        {
            ...
            cfprintf(writedest, "%s %s %s %s %s %s %s %s\n",
                    "_CLOCKS_DATA", "Host", "Socket", "APERF", "MPERF", "TSC",
                    "CurrFreq_MHz", "AvgFreq_MHz");
#else
        ...
        else if (control_domains == CORE)
        {
            ...
            cfprintf(writedest, "%s %s %s %s %s %s %s %s %s %s\n",
                    "_CLOCKS_DATA", "Host", "Socket", "Core", "PhysicalThread",
                    "LogicalThread", "APERF", "MPERF", "TSC", "CurrFreq_MHz",
                    "AvgFreq_MHz");
            ...
            for (i = 0; i < nsockets; i++)
            {
                for (j = 0; j < ncores / nsockets; j++)
                {
                    for (k = 0; k < nthreads / ncores; k++)
                    {
                        idx = (k * nsockets * (ncores / nsockets)) + (i * (ncores / nsockets)) + j;
#ifdef CPRINTF_FOUND
                        cfprintf(writedest, "%s %s %d %d %d %lu %lu %lu %lu %f\n",
