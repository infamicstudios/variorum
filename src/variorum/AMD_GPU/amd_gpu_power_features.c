         ...
         i < (chipid + 1) * gpus_per_socket; i++)
    {
        ...
        else
        {
            ...
            cfprintf(output, "%s %s %d %d %0.2lf %lf\n",
                    "_AMD_GPU_POWER_USAGE", hostname, chipid, i, pwr_val_flt,
                    (now.tv_usec - start.tv_usec) / 1000000.0);
#else
            fprintf(output, "%s %s %d %d %0.2lf %lf\n",
                    "_AMD_GPU_POWER_USAGE", hostname, chipid, i, pwr_val_flt,
                    (now.tv_usec - start.tv_usec) / 1000000.0);
#endif
        }
         ...
         i < (chipid + 1) * gpus_per_socket; i++)
    {
        ...
        {   
            ...
            cfprintf(output, "%s %s %d %d %0.2lf %ld %ld %lf\n",
                    ...
                    (now.tv_usec - start.tv_usec) / 1000000.0);
#else
            fprintf(output, "%s %s %d %d %0.2lf %ld %ld %lf\n",
                    "_AMD_GPU_POWER_CAP", hostname, chipid, i, pwr_val_flt, pwr_min, pwr_max,
                    (now.tv_usec - start.tv_usec) / 1000000.0);
#endif
        }
...
void get_clocks_data(int chipid, int total_sockets, int verbose, FILE *output)
{
         ...
         i < (chipid + 1) * gpus_per_socket; i++)
    {
        ...
        {
#ifdef CPRINTF_FOUND
            cfprintf(output, "%s %s %d %d %d %d %lf\n",
                    "_AMD_GPU_CLOCKS", hostname, chipid, i, f_sys_val, f_mem_val,
                    (now.tv_usec - start.tv_usec) / 1000000.0);
#else
            fprintf(output, "%s %s %d %d %d %d %lf\n",
                    "_AMD_GPU_CLOCKS", hostname, chipid, i, f_sys_val, f_mem_val,
                    (now.tv_usec - start.tv_usec) / 1000000.0);
#endif
        }
